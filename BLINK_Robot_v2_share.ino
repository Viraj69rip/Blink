...
#define CMD_CHAR_UUID    "beb5483e-36e1-4688-b7f5-ea07361b26a9"  // Write: FOCUS / IDLE / W:SUN / CLEAR / SW:...
...
static const float    SHAKE_THRESHOLD   = 1.2f;  // g-force spike (lowered for desk sensitivity)
static const float    SHAKE_DELTA_G     = 0.6f;  // deviation from baseline (lowered — was 1.8)
static const uint32_t SHAKE_COOLDOWN_MS = 600;   // cooldown after shake triggers
...
bool drawMode = false;
char weatherCode[8] = "---";   // e.g. SUN, CLD, RAIN
...
// Long-press detection (2-second hold to exit Clock/HW Info modes)
...
void factoryResetRobot() {
  ...
  menuIndex = 0;
  stopwatchText[0] = '\0';
  strncpy(weatherCode, "---", sizeof(weatherCode));
  weatherCode[sizeof(weatherCode) - 1] = '\0';
...
void drawIdleFace(uint32_t t) {
  ...
  // Display mode overlays are now handled by drawIdleOverlays()
}
...
// Helper: display mode overlays (extracted from drawIdleFace)
void drawIdleOverlays() {
  if (displayMode == DISPLAY_WEATHER &&
      weatherCode[0] != '-' && weatherCode[0] != '\0') {
    u8g2.setFont(u8g2_font_logisoso16_tr);
    int tw = u8g2.getStrWidth(weatherCode);
    u8g2.drawStr((OLED_W - tw) / 2, 58, weatherCode);
    return;
  }
  if (displayMode == DISPLAY_TIME && timeSynced) {
    char clockBuf[8];
    formatLocalTime(clockBuf, sizeof(clockBuf));
    u8g2.setFont(u8g2_font_logisoso22_tr);
    int tw = u8g2.getStrWidth(clockBuf);
    u8g2.drawStr((OLED_W - tw) / 2, 58, clockBuf);
    return;
  }
  if (displayMode == DISPLAY_BLE) {
    u8g2.setFont(u8g2_font_6x12_tr);
    const char* bleTxt = bleConnected ? "BLE: PAIRED" : "BLE: OPEN";
    int tw = u8g2.getStrWidth(bleTxt);
    u8g2.drawStr((OLED_W - tw) / 2, 58, bleTxt);
    return;
  }
  if (displayMode == DISPLAY_DATE && timeSynced) {
    char dateBuf[16];
    formatLocalDate(dateBuf, sizeof(dateBuf));
    u8g2.setFont(u8g2_font_6x12_tr);
    int tw = u8g2.getStrWidth(dateBuf);
    u8g2.drawStr((OLED_W - tw) / 2, 58, dateBuf);
    return;
  }
}
...
void drawMenuScreen() {
  ...
  u8g2.setFont(u8g2_font_4x6_tr);
  u8g2.drawStr(2, 62, "1x dn 2x up 3x pick");
}
...
void applyMenuSelection() {
  ...
  switch (menuIndex) {
    ...
    case 6:  // Mario Clock
      ...
      // Reset Mario clock animation state
      ...
      for (int i = 0; i < 5; i++) { mclock_digitOffY[i] = 0; mclock_digitVelY[i] = 0; }
      enterState(STATE_APP_MODE);
      break;
    case 7:  // HW Info
      focusActive = false;
...
void drawAppModeScreen(uint32_t t) {
  ...
  if (focusActive || displayMode == DISPLAY_FOCUS) {
    // Focus mode: timer ONLY — absolutely nothing else on screen
    const char* timerTxt = stopwatchText[0] != '\0' ? stopwatchText : "25:00";
    u8g2.setFont(u8g2_font_logisoso28_tr);
    int tw = u8g2.getStrWidth(timerTxt);
    int th = 28;  // approximate font height
    int ty = (OLED_H + th) / 2;  // vertically centered
    u8g2.drawStr((OLED_W - tw) / 2, ty, timerTxt);
  ...
  if (displayMode == DISPLAY_WEATHER) {
    u8g2.setFont(u8g2_font_logisoso22_tr);
    const char* w = (weatherCode[0] != '-' && weatherCode[0] != '\0') ? weatherCode : "---";
    int tw = u8g2.getStrWidth(w);
    u8g2.drawStr((OLED_W - tw) / 2, 44, w);
    return;
  }
  ...
  if (displayMode == DISPLAY_TIME && timeSynced) {
    ...
    u8g2.setFont(u8g2_font_logisoso28_tr);
    int tw = u8g2.getStrWidth(clockBuf);
    u8g2.drawStr((OLED_W - tw) / 2, 42, clockBuf);
    // Date below
    char dateBuf[16];
    formatLocalDate(dateBuf, sizeof(dateBuf));
    u8g2.setFont(u8g2_font_6x12_tr);
    int dw = u8g2.getStrWidth(dateBuf);
    u8g2.drawStr((OLED_W - dw) / 2, 58, dateBuf);
    return;
  }
  ...
  if (displayMode == DISPLAY_DATE && timeSynced) {
    ...
    u8g2.setFont(u8g2_font_logisoso16_tr);
    int tw = u8g2.getStrWidth(dateBuf);
    u8g2.drawStr((OLED_W - tw) / 2, 44, dateBuf);
    return;
  }

  // Default app mode: calm blinking face + clock footer
  float breath = sinf(t / 800.0f);
  int bob = (int)(breath * 1.5f);
  uint32_t cycle = t % 3200;
  int eyeH = 14;
  if (cycle > 3200 - 220) {
    float p = (cycle - (3200 - 220)) / 220.0f;
    float lid = (p < 0.5f) ? easeInOut(p * 2.0f) : easeInOut((1.0f - p) * 2.0f);
    eyeH = (int)(14 - lid * 12);
    eyeH = clampi(eyeH, 2, 14);
  }
  drawEyesBase(40, 88, 22 + bob, 16, eyeH, 0);
  u8g2.setFont(u8g2_font_6x12_tr);
  if (timeSynced) {
    char clockBuf[8];
    formatLocalTime(clockBuf, sizeof(clockBuf));
    u8g2.drawStr(48, 44, clockBuf);
  }
}
...
bool readShake() {
  ...
  // Track resting gravity so shake = sudden deviation
  mpuBaselineG = mpuBaselineG * 0.97f + gForce * 0.03f;
  float delta = fabsf(gForce - mpuBaselineG);

  // Detect shake: either absolute threshold or sudden deviation
  bool shook = gForce > SHAKE_THRESHOLD || delta > SHAKE_DELTA_G;
  if (shook) {
    Serial.printf("[MPU] SHAKE! g=%.2f base=%.2f delta=%.2f\n", gForce, mpuBaselineG, delta);
  }
  return shook;
}
...
void handleTouchGesture(int tapCount) {
  ...
  if (tapCount == 1 &&
      displayMode != DISPLAY_MARIO_CLOCK && displayMode != DISPLAY_HW_INFO &&
      (state == STATE_IDLE || (state == STATE_APP_MODE && !drawMode && !focusActive))) {
...
void pollTouchGestures() {
  ...
  // Long-press detection: hold touch for 2 seconds to exit Clock/HW Info
  if (pressed && touchWasActive) {
    uint32_t holdDuration = now - touchDownAt;
    if (holdDuration >= LONG_PRESS_MS && !longPressTriggered) {
      longPressTriggered = true;
      if (displayMode == DISPLAY_MARIO_CLOCK || displayMode == DISPLAY_HW_INFO) {
        // Exit special mode back to normal face
        focusActive = false;
        drawMode = false;
        displayMode = DISPLAY_NORMAL;
        menuOpen = false;
...
void handlePendingBle() {
  ...
  if (pendingCmd) {
    ...
    if (strncmp(cmdBuf, "FOCUS", 5) == 0) {
      ...
      strncpy(weatherCode, cmdBuf + 2, sizeof(weatherCode) - 1);
      weatherCode[sizeof(weatherCode) - 1] = '\0';
...
void setupBle() {
  ...
  stateChar->setValue("0,0,0,0,0,0");
